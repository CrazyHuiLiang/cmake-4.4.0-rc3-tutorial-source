...
"[`CMake Language`](../../manual/cmake-language.7.html#manual:cmake-language(7))", or more colloquially as CMakeLang.

> **Note:** The CMake Language is not well suited to describing things which are not
...
> **Note:** This tutorial endeavors to demonstrate best practices and solutions to real
...
[`list(APPEND)`](../../command/list.html#append).
...
[`set()`](../../command/set.html#command:set) command.
...
to use the [`message()`](../../command/message.html#command:message) command to print the string named by `var`.
...
> **Note:** [`cmake -P`](../../manual/cmake.1.html#cmdoption-cmake-P) is called "script mode", it informs CMake this file is not
intended to have a [`project()`](../../command/project.html#command:project) command. We're not building any
...
to consult the [`if()`](../../command/if.html#command:if) documentation on expressions is worthwhile. It's
recommended to stick to a single pair for a given context, such as
"True"/"False" or "On"/"Off".

As mentioned, lists are strings containing semicolons. The [`list()`](../../command/list.html#command:list)
...
[`foreach()`](../../command/foreach.html#command:foreach) command to iterate over a list.
...
[`function()`](../../command/function.html#command:function) and [`macro()`](../../command/macro.html#command:macro).
...
of scope. In CMakeLang, both [`function()`](../../command/function.html#command:function) and [`macro()`](../../command/macro.html#command:macro) can "see"
all the variables created in all the frames above them. However, a
[`macro()`](../../command/macro.html#command:macro) acts semantically like a text replacement, similar to C/C++
...
[`function()`](../../command/function.html#command:function) creates its own variable scope, so side effects are not
...
same as [`set()`](../../command/set.html#command:set) but for variables belonging to the caller's context.

> **Note:** In CMake 3.25, the [`return(PROPAGATE)`](../../command/return.html#command:return) option was added, which
works the same as [`set(PARENT_SCOPE)`](../../command/set.html#command:set) but provides slightly better
ergonomics.

While not necessary for this exercise, it bears mentioning that [`macro()`](../../command/macro.html#command:macro)
and [`function()`](../../command/function.html#command:function) both support variadic arguments via the `ARGV`
...
construct our own version of [`list(APPEND)`](../../command/list.html#append), which adds a value to a
list.

### Goal

Implement a macro and a function which append a value to a list, without using
the [`list(APPEND)`](../../command/list.html#append) command.
...
> **Note:** The extension for these exercises is `.cmake`, that's the standard extension
...
> **Note:** You're not expected to handle the case of an empty or undefined list to
...
`ListVar` with our knowledge of the [`set()`](../../command/set.html#command:set) command.
...
with [`set(PARENT_SCOPE)`](../../command/set.html#command:set).
...
convention established by the [`if()`](../../command/if.html#command:if) command.

When given a string, [`if()`](../../command/if.html#command:if) will first check if it is one of the known
...
> **Note:** This is a good a time as any to discuss quoting in CMake. All objects in
...
>
> A handful of commands, such as [`if()`](../../command/if.html#command:if), recognize the difference
> between quoted and unquoted strings. [`if()`](../../command/if.html#command:if) will only check that the
> given string represents a variable when the string is unquoted.

Finally, [`if()`](../../command/if.html#command:if) provides several useful comparison modes such as
...
[`while()`](../../command/while.html#command:while), which follows the same rules as [`if()`](../../command/if.html#command:if) for checking a
loop variable, and the more useful [`foreach()`](../../command/foreach.html#command:foreach), which iterates over lists
of strings and was demonstrated in the [Background](#background) section.
...
[`function()`](../../command/function.html#command:function) as the list to operate on.

### Goal

Loop over a list, and return all the strings containing the string `Foo`.

> **Note:** Those who read the command documentation will be aware that this is
[`list(FILTER)`](../../command/list.html#filter), resist the temptation to use it.
...
> **Note:** You should use the [`list(APPEND)`](../../command/list.html#append) command this time to collect your
...
While there are a couple ways we could invoke [`foreach()`](../../command/foreach.html#command:foreach), the
recommended way is to allow the command to do the variable expansion for us
via `IN LISTS` to access the `ARGN` list items.

The [`if()`](../../command/if.html#command:if) comparison we need is `MATCHES` which will check if
...
own CMLs with [`add_subdirectory()`](../../command/add_subdirectory.html#command:add_subdirectory). In later steps we will explore
...
[`include()`](../../command/include.html#command:include) command. This command immediately begins interpreting the
contents of the [`include()`](../../command/include.html#command:include)'d file in the scope of the parent CML. It
...
> **Note:** Actually it reuses tests from Exercise2.cmake, reusable code is good for
everyone.
...
The [`include()`](../../command/include.html#command:include) command will interpret the included file completely,
...
Now we're ready to [`include()`](../../command/include.html#command:include) the previous exercises to grab their
functions.
